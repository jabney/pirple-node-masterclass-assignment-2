...
const postUser = (data, callback) => {
    if (isValidUser(data.payload)) {
        ...
        dataStore.read('users', email, (readError) => {
            ...
            if (fileDoesNotExist) {
                ...
                dataStore.create('users', email, newUser, (createError) => {
                    if (!createError) {
            ...
            if (isTokenValid) {
                dataStore.read('users', email, (err, userData) => {
                    if (!err && userData) {
                        ...
                        dataStore.update('users', email, updatedUserData, (err) => {
                            if (!err) {
    ...
    if (email) {
        ...
        verifyToken(token, email, (isTokenValid) => {
            if (isTokenValid) {
                dataStore.read('users', email, (readError, userData) => {
                    if (!readError && userData) {
                        dataStore.delete('users', email, (deleteError) => {
                            if (!deleteError) {
