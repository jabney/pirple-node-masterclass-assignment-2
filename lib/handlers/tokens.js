        ...
        storeUtils.read('users', email, (readError, userData) => {
            if (!readError && userData) {
                ...
                if (hashedPassword === userData.password) {
                    ...
                    storeUtils.create('tokens', tokenId, tokenObject, (createError) => {
                        if (!createError) {
...
const getToken = (data, callback) => {
    ...
    if (tokenId) {
        storeUtils.read('tokens', tokenId, (getError, tokenData) => {
...
const putToken = (data, callback) => {
    ...
    if (tokenId) {
        storeUtils.read('tokens', tokenId, (readError, tokenData) => {
            if (!readError && tokenData) {
                ...
                if (isTokenValid) {
                    ...
                    storeUtils.update('tokens', tokenId, tokenData, (updateError) => {
                        if (!updateError) {
...
const deleteToken = (data, callback) => {
    ...
    if (tokenId) {
        storeUtils.read('tokens', tokenId, (readError, tokenData) => {
            if (!readError && tokenData) {
                storeUtils.delete('tokens', tokenId, (deleteError) => {
                    if (deleteError) {
...
const verifyToken = (tokenId, email, callback) => {
    storeUtils.read('tokens', tokenId, function (readError, tokenData) {
        if (!readError && tokenData) {
